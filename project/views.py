...
def HOME(request):
    if request.method=="GET":
        ...
        return render(request, 'project/home.html', {"TEST": "User: ", "user1": current_user, "time": date, 'user': ""})
...
def login_views(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.get_user()
            login(request, user)
            return render(request, 'project/home.html', {'user': user, "time": date, "message": "; You have sucessfully logged in!!"})
    else:
        date = datetime.date(datetime.now())
